...
class BeeGame {
  ...
  moveBee(deltaTime) {
    ...
    const right = new window.THREE.Vector3(1, 0, 0);
    right.applyQuaternion(this.bee.quaternion);
    
    // Calculate move direction
    const moveDirection = new window.THREE.Vector3(0, 0, 0);
    ...
    if (moveDirection.length() > 0) {
      ...
      if (moveDirection.x !== 0 || moveDirection.z !== 0) {
        const horizontalDirection = new window.THREE.Vector3(moveDirection.x, 0, moveDirection.z).normalize();
